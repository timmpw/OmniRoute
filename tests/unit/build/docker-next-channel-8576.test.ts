...
// the active release branch without moving :latest. `main` intentionally does
// move :latest on every successful build.
...
test("next and other non-semver channels cannot promote latest via the semver helper", () => {
...
test("main builds always promote latest while next builds do not", () => {
  assert.match(WORKFLOW, /if \[ "\$VERSION" = "main" \]; then\s+PROMOTE="true"/);
  assert.match(WORKFLOW, /elif \[ "\$VERSION" = "next" \]; then\s+PROMOTE="false"/);
});
