...
import { resolveProviderAlias } from "../config/providerAliases.ts";
...
async function resolveModelByProviderInference(modelId: string, extendedContext: boolean) {
  ...
  // provider id through `resolveProviderAlias()`. The compatibility resolver
  // rewrites a raw "opencode" id to "opencode-zen" before it reaches the active set —
